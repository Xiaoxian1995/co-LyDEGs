...
# 0. 相对路径与输出文件夹设置
# ==============================================================================
# 设定相对输出目录（保存在当前工作目录下的 AD_Results 文件夹）
out_dir <- file.path(".", "AD_Consensus_Clustering_Result")
if (!dir.exists(out_dir)) dir.create(out_dir, recursive = TRUE)

# ==============================================================================
# 1. 数据加载与预处理：严格筛选出 AD 患者样本 (相对路径加载)
# ==============================================================================
csv_file <- file.path(".", "AD_5_Genes_Sample_Expression.csv")

if (!file.exists(csv_file)) {
  stop("❌ 未在当前工作目录下找到基因表达数据文件 'AD_5_Genes_Sample_Expression.csv'，请检查路径！")
}


cat(sprintf("📂 成功找到输入文件 (相对路径): %s\n", csv_file))
...
expr_matrix_centered <- t(scale(t(expr_matrix_ad), scale = FALSE))

# ==============================================================================
# 2. 运行一致性聚类 (Consensus Clustering)
# ==============================================================================
cat("\n====================================================================\n")
cat("🧬 正在运行 ConsensusClusterPlus (hc + pearson)... \n")
cat("--------------------------------------------------------------------\n")
...
# 保存单纯亚型结果表 (相对路径)
subtypes_only_csv <- file.path(out_dir, "AD_Patients_Lysosome_Subtypes_Result.csv")
write.csv(ad_subtypes, subtypes_only_csv, row.names = FALSE)

# ==============================================================================
# 3. 执行 PCA 降维并映射 Consensus 亚群
# ==============================================================================
pca_expr_mat <- t(expr_matrix_ad)
...
# 映射聚类结果
subtype_map <- setNames(ad_subtypes$Lysosome_Subtype, ad_subtypes$Sample_ID)
...
p_native_pca <- ggplot(pca_coordinates, aes(x = PC1, y = PC2, color = Subtype, fill = Subtype)) +
  ...
  theme_bw() +
  labs(
    title = "PCA of Lysosome-Related Subtypes (AD Cohort)",
...
pdf_path <- file.path(out_dir, "PCA_AD_Lysosome_Consensus_ExactReproduce.pdf")
...
final_combined_df <- ad_data %>%
  select(all_of(sample_col), all_of(group_col), all_of(cluster_genes)) %>%
  left_join(ad_subtypes, by = setNames("Sample_ID", sample_col)) %>%
  cbind(pca_scores[match(ad_data[[sample_col]], rownames(pca_scores)), ])
...
output_csv <- file.path(out_dir, "AD_Lysosome_Subtypes_With_Expression_and_PCA.csv")
write.csv(final_combined_df, output_csv, row.names = FALSE)

cat("\n====================================================================\n")
cat("🎉 【运行完成】！全部文件已成功相对保存至目标文件夹：\n")
