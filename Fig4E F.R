# ==============================================================================
# 🎯 [PD 免疫分析一体化脚本] 热图 + Wilcoxon分面箱线图 + 全差异排序CSV (相对路径版)
# ==============================================================================
library(pheatmap)
library(ggplot2)
library(dplyr)
...
cat("📊 1. 正在读取与处理 PD 矩阵数据 (使用相对路径)...\n")

# ------------------------------------------------------------------------------
# 0. 相对路径定义与数据读取
# ------------------------------------------------------------------------------
pd_csv_path     <- "PD_Lysosome_Subtypes_With_Expression_and_PCA.csv"
ssgsea_csv_path <- "pd_ssgsea_23_scores.csv"

if (!file.exists(pd_csv_path)) {
  stop(sprintf("⚠️ 错误：未在当前目录下找到 PD 亚型数据文件：%s", pd_csv_path))
}
if (!file.exists(ssgsea_csv_path)) {
  stop(sprintf("⚠️ 错误：未在当前目录下找到 PD ssGSEA 免疫得分数据文件：%s", ssgsea_csv_path))
}

df_pd_group_raw  <- read.csv(pd_csv_path, stringsAsFactors = FALSE, header = TRUE)
df_ssgsea_scores <- read.csv(ssgsea_csv_path, stringsAsFactors = FALSE, header = TRUE)

# ---- [第一步：提取并规范 PD 样本 ID] ----
df_pd_group <- df_pd_group_raw
if ("Sample_ID" %in% colnames(df_pd_group)) {
  df_pd_group$Final_Sample_ID <- as.character(df_pd_group$Sample_ID)
} else if ("X" %in% colnames(df_pd_group)) {
  df_pd_group$Final_Sample_ID <- as.character(df_pd_group$X)
} else {
  df_pd_group$Final_Sample_ID <- rownames(df_pd_group)
}
...
if (any(df_pd_group$Final_Sample_ID %in% rownames(df_immune))) {
  mat_cells_pure <- t(as.matrix(df_immune))
} else if (any(df_pd_group$Final_Sample_ID %in% colnames(df_immune))) {
...
# ---- [第三步：样本对齐与横向排序] ----
common_samples <- intersect(df_pd_group$Final_Sample_ID, colnames(mat_cells_pure))
...
if (length(common_samples) == 0) {
  stop("⚠️ 错误：PD 亚型表与 ssGSEA 免疫表的样本 ID 无法匹配！")
}

df_pd_aligned <- df_pd_group[df_pd_group$Final_Sample_ID %in% common_samples, ]
df_pd_aligned <- df_pd_aligned[order(df_pd_aligned$Lysosome_Subtype), ]
extracted_subtype <- as.character(df_pd_aligned$Lysosome_Subtype)

mat_aligned <- mat_cells_pure[, df_pd_aligned$Final_Sample_ID, drop = FALSE]

# ------------------------------------------------------------------------------
# 🎨 1. 生成并导出 AI 可编辑的 PD 免疫热图 PDF
# ------------------------------------------------------------------------------
cat("🎨 2. 正在导出 PD 免疫热图 PDF...\n")
...
output_heatmap_pdf <- "PD_Lysosome_Subtypes_Immune_Infiltration_AIVersion.pdf"
...
pheatmap(
  ...
  main              = "Immune Infiltration Landscape (PD Subtypes)"
)
dev.off()
...
merged_df <- inner_join(
  df_pd_aligned %>% select(Final_Sample_ID, Lysosome_Subtype),
  mat_df,
  by = "Final_Sample_ID"
...
output_csv_path <- "PD_All_Immune_Cells_Differential_Ranked.csv"
...
output_boxplot_pdf <- "Figure_PD_Immune_Infiltration_SolidPoints_AI.pdf"

pdf(output_boxplot_pdf, width = canvas_width, height = 5.0, family = "sans", useDingbats = FALSE)
print(
  wrap_plots(plot_list, nrow = 1) +
    plot_annotation(
      title = "Top Significant Immune Cell Infiltration In PD Subtypes",
