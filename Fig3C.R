# ==============================================================================
# 🎯 [AD 溶酶体核心基因亚型差异表达分析·相对路径+修复 parse 报错版]
# ==============================================================================
library(ggplot2)
library(dplyr)
library(tidyr)
...
# ------------------------------------------------------------------------------
# 2. 增强版 P 值 plotmath 格式化函数（防 0/Inf 报错）
# ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
cat("\n====================================================================\n")
cat("📊 【正在绘制两个亚型间的溶酶体基因差异表达图...】\n")
cat("--------------------------------------------------------------------\n")

for (g in available_genes) {
  d_sub <- ad_plot_data %>% filter(Gene == g)
  ...
  # 导出 PDF 至相对路径
  pdf_path <- file.path(".", paste0("Subtype_Diff_", g, ".pdf"))
  ggsave(pdf_path, p, width = 4, height = 4.8)
  
  cat(sprintf("✅ 已生成亚型差异图至相对路径: %s (Raw p = %.4e)\n", pdf_path, p_val))
}
