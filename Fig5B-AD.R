# ==============================================================================
# 🎯 [AD 双亚型特征评分 ROC 曲线 - 相对路径版]
# ==============================================================================
library(pROC)
library(ggplot2)

cat("📊 1. 正在读取 AD 本地 CSV 数据并计算 Signature ROC...\n")

# 使用纯相对路径获取当前工作目录下的文件
ad_csv_path <- "AD_Lysosome_Subtypes_With_Expression_and_PCA.csv"

if (!file.exists(ad_csv_path)) {
  stop("⚠️ 错误：未在当前工作目录下找到 AD_Lysosome_Subtypes_With_Expression_and_PCA.csv 文件！")
}

# 1. 载入原始数据
ad_roc_data <- read.csv(ad_csv_path, stringsAsFactors = FALSE, header = TRUE)

# 2. 自动构建二分类响应变量 (Group_Binary)
subtype_col <- NULL
for (col in c("Lysosome_Subtype", "Subtype", "Cluster")) {
  if (col %in% colnames(ad_roc_data)) {
    subtype_col <- col
    break
  }
}
...
raw_subtypes <- as.character(ad_roc_data[[subtype_col]])
ad_roc_data$Group_Binary <- ifelse(grepl("1", raw_subtypes), 1, 0)

cat(sprintf("💡 已成功将 '%s' 列转化为二分类标签 (样本总数: %d)\n", subtype_col, nrow(ad_roc_data)))

# 3. 🔍 定义正确的基因分组
genes_A <- c("SLC7A14", "CLTC")
genes_B <- c("SLC2A13", "ACP5", "RAMP3")

# 检查基因是否存在于列名中
missing_A <- setdiff(genes_A, colnames(ad_roc_data))
missing_B <- setdiff(genes_B, colnames(ad_roc_data))
if (length(missing_A) > 0 || length(missing_B) > 0) {
  warning("⚠️ 提示：部分指定基因在数据框列名中未完全找到，请核对拼写！")
}

# 4. ⚡️ 自动在数据框中计算 Signature 评分（求行均值）
valid_genes_A <- intersect(genes_A, colnames(ad_roc_data))
valid_genes_B <- intersect(genes_B, colnames(ad_roc_data))

ad_roc_data$AD_Signature_A <- rowMeans(ad_roc_data[, valid_genes_A, drop = FALSE], na.rm = TRUE)
ad_roc_data$AD_Signature_B <- rowMeans(ad_roc_data[, valid_genes_B, drop = FALSE], na.rm = TRUE)

# 5. 📈 计算对应的 ROC 曲线
roc_A_ad <- roc(response = ad_roc_data$Group_Binary, 
                predictor = ad_roc_data$AD_Signature_A, 
                quiet = TRUE)

roc_B_ad <- roc(response = ad_roc_data$Group_Binary, 
                predictor = ad_roc_data$AD_Signature_B, 
                quiet = TRUE)

# 6. 提取 ROC 坐标点并转化为标准 FPR (1 - Specificity)
coords_A <- coords(roc_A_ad, "all", ret = c("specificity", "sensitivity"), transpose = FALSE)
coords_B <- coords(roc_B_ad, "all", ret = c("specificity", "sensitivity"), transpose = FALSE)

df_A <- data.frame(
  FPR = 1 - coords_A$specificity,
  TPR = coords_A$sensitivity,
  Signature = "Signature A (SLC7A14, CLTC)"
)
...
df_B <- data.frame(
  ...
  Signature = "Signature B (SLC2A13, ACP5, RAMP3)"
)
...
# 获取精确 AUC 值
auc_A_val <- sprintf("%.4f", as.numeric(auc(roc_A_ad))) 
auc_B_val <- sprintf("%.4f", as.numeric(auc(roc_B_ad))) 

cat("🎨 2. 正在生成 AD 学术级 ROC 曲线图...\n")
...
p_roc_ad <- ggplot() +
  ...
  scale_color_manual(values = c(
    "Signature A (SLC7A14, CLTC)" = "#C87A2F",
    "Signature B (SLC2A13, ACP5, RAMP3)" = "#1F4E79"
  )) +
...
# ------------------------------------------------------------------------------
output_pdf <- "Figure_AD_Merged_Signatures_ROC_Editable.pdf"
cat(sprintf("💾 3. 正在生成高保真可编辑的 AD 矢量 PDF：%s\n", output_pdf))
