# ==============================================================================
# 🎯 [PD 溶酶体 Signature Score 差异分析·相对路径+精准 plotmath P 值版]
# ==============================================================================
library(ggplot2)
library(dplyr)
library(tidyr)
...
# ------------------------------------------------------------------------------
# 1. 计算 PD Signature A 与 Signature B 得分
# ------------------------------------------------------------------------------
# 检查所需基因是否存在
required_genes <- c("ACP5", "RAMP3", "SLC2A13", "CLTC", "SLC7A14")
missing_genes  <- setdiff(required_genes, colnames(df_raw))

if (length(missing_genes) > 0) {
  stop(sprintf("⚠️ 错误：PD 数据集中缺失以下核心基因：%s", paste(missing_genes, collapse = ", ")))
}

pd_score_data <- df_raw %>%
  mutate(
    # Signature A (ACP5 & RAMP3)
    Signature_Score_A = (ACP5 + RAMP3) / 2,
    # Signature B (SLC2A13 & CLTC & SLC7A14)
    Signature_Score_B = (SLC2A13 + CLTC + SLC7A14) / 3,
...
# ------------------------------------------------------------------------------
# 🎨 3. 绘制 PD Signature A 得分对比图 (ACP5 & RAMP3)
# ------------------------------------------------------------------------------
p_val_A      <- t.test(Signature_Score_A ~ Lysosome_Subtype, data = pd_score_data)$p.value
p_expr_str_A <- format_p_plotmath(p_val_A)

max_y_A   <- max(pd_score_data$Signature_Score_A, na.rm = TRUE)
min_y_A   <- min(pd_score_data$Signature_Score_A, na.rm = TRUE)
y_range_A <- max_y_A - min_y_A

p_A <- ggplot(pd_score_data, aes(x = Lysosome_Subtype, y = Signature_Score_A)) +
  ...
  labs(title = "Lysosome Subtype Signature A\n(ACP5 & RAMP3 enriched)", x = "", y = "Signature Score") +
...
# 导出至相对路径
pdf_path_A <- file.path(".", "PD_Subtype_Signature_Score_A.pdf")
ggsave(pdf_path_A, p_A, width = 4, height = 4.8)

# ------------------------------------------------------------------------------
# 🎨 4. 绘制 PD Signature B 得分对比图 (SLC2A13 & CLTC & SLC7A14)
# ------------------------------------------------------------------------------
p_val_B      <- t.test(Signature_Score_B ~ Lysosome_Subtype, data = pd_score_data)$p.value
p_expr_str_B <- format_p_plotmath(p_val_B)

max_y_B   <- max(pd_score_data$Signature_Score_B, na.rm = TRUE)
min_y_B   <- min(pd_score_data$Signature_Score_B, na.rm = TRUE)
y_range_B <- max_y_B - min_y_B

p_B <- ggplot(pd_score_data, aes(x = Lysosome_Subtype, y = Signature_Score_B)) +
  geom_rect(aes(xmin = 0.5, xmax = 1.5, ymin = -Inf, ymax = Inf), fill = "#E5E7EB", alpha = 0.3) +
  geom_rect(aes(xmin = 1.5, xmax = 2.5, ymin = -Inf, ymax = Inf), fill = "#FECACA", alpha = 0.3) +
  geom_jitter(width = 0.15, size = 1.3, alpha = 0.7, color = "black") +
  geom_boxplot(width = 0.3, fill = NA, color = "black", outlier.shape = NA, linewidth = 0.8) +
  annotate("text", x = 1.5, y = max_y_B + y_range_B * 0.1, label = p_expr_str_B, parse = TRUE, size = 5) +
  labs(title = "Lysosome Subtype Signature B\n(SLC2A13 & CLTC & SLC7A14 enriched)", x = "", y = "Signature Score") +
  scale_y_continuous(expand = expansion(mult = c(0.06, 0.18))) +
  theme_classic() +
  theme(
    plot.title   = element_text(hjust = 0.5, size = 14, face = "bold"),
...
# 导出至相对路径
pdf_path_B <- file.path(".", "PD_Subtype_Signature_Score_B.pdf")
...
# ------------------------------------------------------------------------------
cat("\n====================================================================\n")
cat("🎉 【PD 数据集 Signature Score 差异分析与相对路径出图成功！】\n")
cat("--------------------------------------------------------------------\n")
cat(sprintf(" 1. PD Signature A 图形: %s (Raw p = %.4e)\n", pdf_path_A, p_val_A))
cat(sprintf(" 2. PD Signature B 图形: %s (Raw p = %.4e)\n", pdf_path_B, p_val_B))
cat("====================================================================\n")